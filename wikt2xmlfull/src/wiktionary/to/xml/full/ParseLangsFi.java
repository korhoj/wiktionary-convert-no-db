...
public class ParseLangsFi {
	...
	private final static Logger LOGGER = Logger.getLogger(ParseLangsFi.class
			.getName());
	...
	static {
		try {
			fileHandler = new FileHandler(ParseLangsFi.class.getName() + ".log");
		} catch (Exception e) {
			System.err.println(""
	...
	public static void main(String[] args) {
		...
		try {
			if (args.length != 2) {
				...
				LOGGER.log(Level.SEVERE, "  ParseLangs_fi Input_filename Output_filename");
...
<td><a href="//fi.wikipedia.org/wiki/fi:Abhaasin_kieli" class="extiw" title="w:fi:Abhaasin kieli">abhaasi</a> Wikipediassa, vapaassa tietosanakirjassa</td>
				...
				case 1:
					...
					if (nameStart > -1) {
						...
						if (name.length() > 1)
							name = name.substring(0,1).toUpperCase(new Locale("fi","FI")) + name.substring(1);
						else if (name.trim().length() == 1)
							name = name.substring(0,1).toUpperCase(new Locale("fi","FI"));
